...
import { Table, Terminal, AlertTriangle, Download, Copy, Check, ChevronUp, ChevronDown } from 'lucide-react';
...
function ResultsTable() {
  ...
  const handleCopy = () => {
    const csv = [queryResult.columns.join('\t'), ...queryResult.rows.map((r) => queryResult.columns.map((c) => String(r[c] ?? '')).join('\t'))].join('\n');
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full text-xs font-mono">
          <thead className="sticky top-0 bg-panel-bg z-10">
            <tr>
              <th className="px-3 py-1.5 text-left text-muted-foreground font-medium border-b border-panel-border w-10">#</th>
              {queryResult.columns.map((col) => (
                <th key={col} className="px-3 py-1.5 text-left text-muted-foreground font-medium border-b border-panel-border whitespace-nowrap">
                  {col}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {queryResult.rows.map((row, i) => (
              <tr key={i} className="hover:bg-secondary/30 transition-colors">
                <td className="px-3 py-1 text-muted-foreground border-b border-panel-border/50">{i + 1}</td>
                {queryResult.columns.map((col) => (
                  <td key={col} className="px-3 py-1 text-foreground border-b border-panel-border/50 whitespace-nowrap">
                    {row[col] === null ? (
                      <span className="text-muted-foreground italic">NULL</span>
                    ) : typeof row[col] === 'boolean' ? (
                      <span className={cn(row[col] ? 'text-success' : 'text-destructive')}>{String(row[col])}</span>
                    ) : (
                      String(row[col])
                    )}
                  </td>
...
export function ResultsPanel() {
  ...
  return (
    <div className="flex flex-col h-full bg-panel-bg">
      <div className="flex items-center border-t border-panel-border shrink-0">
