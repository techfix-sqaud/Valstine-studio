...
exports.registerDbIPC = registerDbIPC;
...
function initAppDb() {
    ...
    appDb = new better_sqlite3_1.default(node_path_1.default.join(userDataPath, 'valstine.db'));
    appDb.pragma('journal_mode = WAL');
    // Load/create master key BEFORE schema work — needed for migration encryption.
    const vaultDir = node_path_1.default.join(userDataPath, 'vault');
    (0, node_fs_1.mkdirSync)(vaultDir, { recursive: true });
    const keyPath = node_path_1.default.join(vaultDir, 'master.key');
    if ((0, node_fs_1.existsSync)(keyPath)) {
        MASTER_KEY = Buffer.from((0, node_fs_1.readFileSync)(keyPath, 'utf8').trim(), 'hex');
    }
    else {
        const key = (0, node_crypto_1.randomBytes)(32);
        (0, node_fs_1.writeFileSync)(keyPath, key.toString('hex'), { mode: 0o600 });
        MASTER_KEY = key;
    }
...
function encrypt(plaintext) {
    ...
    const cipher = (0, node_crypto_1.createCipheriv)('aes-256-gcm', MASTER_KEY, iv);
...
function decrypt(stored) {
    ...
    try {
        ...
        const decipher = (0, node_crypto_1.createDecipheriv)('aes-256-gcm', MASTER_KEY, iv);
...
function getKnex(c) {
    ...
    const instance = (0, knex_1.default)(config);
...
function dockerEnv() {
    ...
    return {
        ...process.env,
        PATH: [...extraPaths, process.env.PATH ?? ''].filter(Boolean).join(node_path_1.default.delimiter),
    };
}
// ── IPC registration ───────────────────────────────────────────────────
function registerDbIPC() {
    ...
    electron_1.ipcMain.handle('app:request', async (_, { method, path: reqPath, body }) => {
        try {
    ...
    // ── DB operations ────────────────────────────────────────────────────
    // provision
    electron_1.ipcMain.handle('db:provision', async (_, payload) => {
        if (payload.type === 'sqlite') {
            ...
            try {
                ...
                const sqliteDb = new better_sqlite3_1.default(filePath);
                sqliteDb.close();
                return {
                    ok: true,
                    connection: {
        ...
        try {
            ...
            const dockerResult = (0, node_child_process_1.spawnSync)(dockerExecutable, dockerArgs, {
                encoding: 'utf-8',
                timeout: 60000,
                env: dockerEnv(),
            });
    ...
    // test-connection
    electron_1.ipcMain.handle('db:test-connection', async (_, payload) => {
    ...
    // disconnect
    electron_1.ipcMain.handle('db:disconnect', async (_, payload) => {
        const connection = resolveConnection(payload);
        destroyKnex(connection);
        if (payload.connectionId) {
            appDb.prepare("UPDATE connections SET status = 'disconnected' WHERE id = ?").run(payload.connectionId);
    ...
    // execute
    electron_1.ipcMain.handle('db:execute', async (_, payload) => {
    ...
    // databases
    electron_1.ipcMain.handle('db:databases', async (_, payload) => {
        const connection = resolveConnection(payload);
        const databases = await listDatabases(connection);
        return { databases };
    });
    // schemas
    electron_1.ipcMain.handle('db:schemas', async (_, payload) => {
        const connection = resolveConnection(payload);
        const schemas = await listSchemas(connection);
        return { schemas };
    });
    // tables
    electron_1.ipcMain.handle('db:tables', async (_, payload) => {
        const connection = resolveConnection(payload);
        const tables = await listTables(connection, payload.schema);
        return { tables };
    });
    // columns
    electron_1.ipcMain.handle('db:columns', async (_, payload) => {
        const connection = resolveConnection(payload);
        const columns = await listColumns(connection, payload.table, payload.schema);
        return { columns };
    });
    // row-count
    electron_1.ipcMain.handle('db:row-count', async (_, payload) => {
        const connection = resolveConnection(payload);
    ...
    // indexes
    // schema-indexes
    electron_1.ipcMain.handle('db:schema-indexes', async (_, payload) => {
        const connection = resolveConnection(payload);
        const indexes = await listSchemaIndexes(connection, payload.schema);
        return { indexes };
    });
    // indexes (per-table)
    electron_1.ipcMain.handle('db:indexes', async (_, payload) => {
        const connection = resolveConnection(payload);
        const indexes = await listIndexes(connection, payload.table, payload.schema);
        return { indexes };
    });
    // functions
    electron_1.ipcMain.handle('db:functions', async (_, payload) => {
        const connection = resolveConnection(payload);
        const functions = await listFunctions(connection, payload.schema);
        return { functions };
    });
    // triggers
    electron_1.ipcMain.handle('db:triggers', async (_, payload) => {
        const connection = resolveConnection(payload);
        const triggers = await listTriggers(connection, payload.schema);
        return { triggers };
    });
    // sequences
    electron_1.ipcMain.handle('db:sequences', async (_, payload) => {
        const connection = resolveConnection(payload);
        const sequences = await listSequences(connection, payload.schema);
        return { sequences };
    });
    // create-database
    electron_1.ipcMain.handle('db:create-database', async (_, payload) => {
    ...
    // drop-database
    electron_1.ipcMain.handle('db:drop-database', async (_, payload) => {
        if (!payload.name?.trim())
            return { ok: false, error: 'Database name required' };
        validateIdentifier(payload.name, 'database name');
        const connection = resolveConnection(payload);
    ...
    // drop-table
    electron_1.ipcMain.handle('db:drop-table', async (_, payload) => {
    ...
    // truncate-table
    electron_1.ipcMain.handle('db:truncate-table', async (_, payload) => {
        validateIdentifier(payload.table, 'table name');
        if (payload.schema)
            validateIdentifier(payload.schema, 'schema name');
        const connection = resolveConnection(payload);
    ...
    // create-schema
    electron_1.ipcMain.handle('db:create-schema', async (_, payload) => {
    ...
    // schema-diff
    electron_1.ipcMain.handle('db:schema-diff', async (_, payload) => {
    ...
    electron_1.ipcMain.handle('git:branches', async () => {
        if (!isGitRepo())
            return { ok: false, error: 'Not a git repository' };
    ...
    electron_1.ipcMain.handle('git:diff', async (_, { file }) => {
        if (!isGitRepo())
            return { ok: false, error: 'Not a git repository' };
    ...
    electron_1.ipcMain.handle('git:stage', async (_, { files }) => {
        if (!isGitRepo())
            return { ok: false, error: 'Not a git repository' };
    ...
    electron_1.ipcMain.handle('git:unstage', async (_, { files }) => {
        if (!isGitRepo())
            return { ok: false, error: 'Not a git repository' };
        for (const f of files)
    ...
    electron_1.ipcMain.handle('git:push', async (_, { branch, setUpstream }) => {
        if (!isGitRepo())
            return { ok: false, error: 'Not a git repository' };
        const current = git(['rev-parse', '--abbrev-ref', 'HEAD']);
    ...
    electron_1.ipcMain.handle('git:checkout', async (_, { branch, create }) => {
        if (!isGitRepo())
            return { ok: false, error: 'Not a git repository' };
        if (!branch?.trim())
    ...
    electron_1.ipcMain.handle('git:log', async (_, { limit }) => {
        if (!isGitRepo())
            return { ok: false, error: 'Not a git repository' };
    ...
    electron_1.ipcMain.handle('ai:chat', async (_, { messages }) => {
    ...
    electron_1.ipcMain.handle('git:create-pr', async (_, { title, body, base }) => {
        if (!isGitRepo())
            return { ok: false, error: 'Not a git repository' };
        let remoteUrl = '';
        try {
